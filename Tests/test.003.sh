...
# but workload runs for 10 seconds of CPU time instead of wall-clock time
...
/bin/su -s /bin/bash -c "taskset -c $X ./work2 10" testuser1 &
/bin/su -s /bin/bash -c "taskset -c $Y ./work2 10" testuser2 &
